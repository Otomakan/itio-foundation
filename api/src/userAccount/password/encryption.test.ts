...
describe('password', () => {
	it('encrypt should match hash', async() => {
		const res = await encrypt(sampleString)
	...
	it('compare should return true when compared to encrypted password', async() => {
		const salt = await bcrypt.genSalt(10)
		const encryptedPassword = await bcrypt.hash(sampleString, salt)
		const res = await compare(sampleString, encryptedPassword)
